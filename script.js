...
// Tutup menu otomatis saat klik link di HP
...
// ===== Simulasi Cuci Tangan 20 Detik =====
const simBtn = document.getElementById('simBtn'),
  simBar = document.getElementById('simProgress'),
  simTime = document.getElementById('simTime');

simBtn.addEventListener('click', () => {
  let t = 20;
  simBtn.disabled = true;
  simBtn.textContent = "⏱️ Simulasi...";
  const interval = setInterval(() => {
    t--;
    simBar.style.width = `${((20 - t) / 20) * 100}%`;
    simTime.textContent = t > 0 ? t + "s" : "Selesai";
    if (t <= 0) {
      clearInterval(interval);
      alert("✅ Simulasi selesai!");
      simBtn.textContent = "Mulai Simulasi 20s";
...
const quiz = [
  { q: "Apa kepanjangan CTPS?", opts: ["Cuci Tangan Pakai Sabun", "Cuci Tubuh Pakai Sabun"], a: 0 },
  { q: "Berapa lama mencuci tangan efektif?", opts: ["10 detik", "20 detik"], a: 1 },
  { q: "Kapan waktu penting mencuci tangan?", opts: ["Sebelum makan dan setelah toilet", "Setelah tidur"], a: 0 }
];
...
function renderQuiz() {
  if (cur >= quiz.length) {
    qEl.textContent = `Selesai! Skor kamu ${score}/${quiz.length}`;
    optEl.innerHTML = "";
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Tombol “Kerjakan Quiz” di halaman home
